...
class TicketDesk:
    def __init__(self, clock: Clock, events: EventLog) -> None:
        self.clock = clock
        self.events = events
        self._tickets: dict[str, Ticket] = {}

    def open(self, merchant_id: str, topic: str, statement: str, records_showed: str) -> Ticket:
        ticket = Ticket(f"TKT-{len(self._tickets) + 1:05d}", merchant_id, topic, statement.strip()[:500],
                        records_showed, self.clock.now().isoformat())
        ...
        self.events.append(Actor.SYSTEM, "ticket.opened", merchant_id=merchant_id, ticket_id=ticket.ticket_id,
                           ...
                           reason="Merchant reported an issue the records do not show; routed to Paytm's team")
        return ticket
